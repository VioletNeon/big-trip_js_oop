  ...
  if (typeOffers.length > 0) {
    const templates = typeOffers.map(({title, price}) => {
      ...
      <div class="event__offer-selector">
        <input class="event__offer-checkbox  visually-hidden" id="event-offer-${offerTitle}-1" data-title="${title}" data-price="${price}" type="checkbox" name="event-offer-${offerTitle}">
